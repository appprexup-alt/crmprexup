...
// Eliminar herramientas Postgres (que salieron con signo de pregunta)
workflow.nodes = workflow.nodes.filter(node => !['toolsearch_pg', 'toolcreate_pg'].includes(node.id));

const tools = [
    {
        id: "toolsearch",
        name: "buscar_propiedades",
        description: "Busca terrenos y propiedades. Parámetros: location (ej: Arequipa), max_price (número).",
        // Ponemos la apikey directamente en la URL para evitar problemas de headers
        url: `={{ "${SUPABASE_URL}/rest/v1/properties?apikey=${ANON_JWT}&select=id,description,location,price,currency,area&status=ilike.Disponible" + ($json.location ? "&location=ilike.*" + encodeURIComponent($json.location) + "*" : "") + ($json.max_price ? "&price=lte." + $json.max_price : "") }}`,
        schema: {
            "type": "object",
            "properties": {
    ...
    },
    {
        ...
        url: `${SUPABASE_URL}/rest/v1/rpc/create_appointment_with_agent?apikey=${ANON_JWT}`,
        method: "POST",
        body: `={{ JSON.stringify({
...
console.log('Main workflow restored with toolHttpRequest and URL-based apikey');
